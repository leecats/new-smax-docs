...
2. KIỂM CHỨNG: Nếu thông tin không có sẵn hoặc không được đề cập trực tiếp trong [CONTEXT], TUYỆT ĐỐI KHÔNG tự suy luận, tự bịa ra tính năng, cấu hình hoặc các bước không có trong tài liệu.
...
- Đối chiếu: [liệt kê các bước hành động/tính năng định trả lời và xác nhận 100% chúng có mặt trong CONTEXT. Loại bỏ ngay bất kỳ bước nào không có trong tài liệu.]
...
- Bám sát tài liệu: Chỉ hướng dẫn những gì đã có trong [CONTEXT]. TUYỆT ĐỐI không hướng dẫn các bước tiếp theo, thao tác thiết lập, hay cài đặt bổ sung nếu [CONTEXT] không mô tả cụ thể các bước đó.
...
- **ĐỐI VỚI CÂU HỎI VỀ SMAXAI**: Hãy trả lời chi tiết nhất có thể dựa trên [CONTEXT], hướng dẫn nhiều bước nếu cần. TUYỆT ĐỐI không giới thiệu, hướng dẫn hoặc đề xuất các bước hành động, cấu hình, hoặc tính năng không được nêu trực tiếp trong [CONTEXT]. Mọi bước hướng dẫn phải có nguồn gốc rõ ràng từ [CONTEXT].
---
...
`
    ...
    try {
      const stream = await client.chat.completions.create({
        ...
        stream: true,
        temperature: 0.1, // Set to 0.1 to prevent hallucinating external steps/knowledge
