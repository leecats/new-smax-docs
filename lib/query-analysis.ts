...
- Đưa ra khuyến nghị dựa trên nhu cầu người dùng nhưng chỉ dựa vào các điểm mạnh có thật trong tài liệu.
- Tuyệt đối không tự suy đoán, so sánh các tính năng hoặc khía cạnh không có thông tin trong tài liệu.
- Cung cấp link tài liệu cho cả hai đối tượng so sánh
`
    case 'how_to':
      return `
# Lưu ý đặc biệt cho câu hỏi hướng dẫn:
- Trả lời theo từng bước rõ ràng (Bước 1, Bước 2, ...). Chỉ hướng dẫn các bước có nguồn gốc rõ ràng từ CONTEXT. Không tự thêm/vẽ ra các bước trung gian hoặc bước tiếp theo nếu tài liệu không nhắc tới.
...
- Chỉ đưa ra các nguyên nhân và giải pháp được nêu trực tiếp trong tài liệu. Không tự suy đoán bừa bãi về lỗi hệ thống.
- Cảnh báo về các rủi ro nếu có trong tài liệu.
- Đề xuất liên hệ support nếu không giải quyết được hoặc tài liệu không đề cập cách xử lý.
`
    case 'feature_inquiry':
      return `
