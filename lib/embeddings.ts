...
4. Giữ nguyên nội dung chính, chỉ cải thiện format và sửa lỗi
5. Trả về văn bản đã được xử lý
6. TUYỆT ĐỐI LOẠI BỎ các bước hướng dẫn, đề xuất hành động tiếp theo, hoặc thông tin kỹ thuật KHÔNG xuất hiện trong CONTEXT. Thà cung cấp ít bước hơn nhưng chính xác hơn là bổ sung các bước tự suy diễn.
...
`
    ...
    const apiResponse = await fetch(`${POST_PROCESSING_BASE_URL}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.1, // Low temperature for strictly grounded and consistent output
        max_tokens: 2048
      })
    })
